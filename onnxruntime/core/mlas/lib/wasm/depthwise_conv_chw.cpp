    ...
    )
{
    // {
    //     auto* output = Output;
    //     for (auto f = 0; f < Parameters->FilterCount; f++) {
    //         std::fill_n(output, Parameters->OutputSize, Bias[f]);
    ...
    for (auto input_channel = Parameters->InputChannels; input_channel > 0; input_channel--) {
        const auto* filter = Filter;
        auto* output = Output;
        for (auto out_channel = Parameters->FilterCount; out_channel > 0; out_channel--) {
            if (input_channel == Parameters->InputChannels) {
                MlasConvSingleChannelAccumulate_CHW<false>(Parameters, Input, filter, output, Zeros);
